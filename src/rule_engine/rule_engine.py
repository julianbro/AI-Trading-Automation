...
class RuleEngine:
    ...
    def _check_breakout_retest(self, market_data: Dict[str, MarketData]) -> Optional[SetupEvent]:
        """
        Check for breakout retest pattern.
        ...
        try:
            ...
            daily_df = pd.DataFrame(daily_data.ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
            intraday_df = pd.DataFrame(intraday_data.ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
    ...
    def _check_support_bounce(self, market_data: Dict[str, MarketData]) -> Optional[SetupEvent]:
        """
        Check for support bounce pattern.
